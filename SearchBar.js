...
//seach bar --> input event --> filter users by name and bio
let inp= document.querySelector("input");
inp.addEventListener("input",(dets)=>
{    let filtered_users = users.filter((user)=>{
        return user.name.toLowerCase().includes(dets.target.value.toLowerCase()) || user.bio.toLowerCase().includes(dets.target.value.toLowerCase())
    })
    container.innerHTML="";
    showUser(filtered_users);
});        
